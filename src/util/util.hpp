...
{
  DWORD flOldProtect;
  VirtualProtect (m_dst, sizeof(m_content), PAGE_EXECUTE_READWRITE, &flOldProtect);
  BOOL r = WriteProcessMemory (GetCurrentProcess (), m_dst, &src, sizeof(src), NULL);
  assert(r);
  VirtualProtect (m_dst, sizeof(m_content), flOldProtect, &flOldProtect);
}

class util::SimpleInlineHook {
  static const constexpr auto mov_rax = make_bytes(0x48, 0xb8);
  static const constexpr auto jmp_rax = make_bytes(0xff, 0xe0);
  Patch<decltype(concat(mov_rax, uintptr_t{0}, jmp_rax))> patch;
public:
  constexpr
  SimpleInlineHook (uintptr_t base, uintptr_t target_func, uintptr_t callback_func)
    : patch{base, target_func, concat(mov_rax, callback_func, jmp_rax)}
  {}
...
public:
  ...
  constexpr
  T trampoline () { return reinterpret_cast<T>(m_dst); }
...
public:
  ...
  constexpr
  T trampoline () { return reinterpret_cast<T>(patch.old_content ()); }
};
